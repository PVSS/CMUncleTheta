...
class KiteApp:
    ...
    def ltp(self, instruments):
        res = self.session.get(f"{self.root_url}/quote/ltp", params={"i": instruments}, headers=self.headers).json()
        print(res)
        data = res["data"]
        return data
    ...
    def place_order(self, variety, exchange, tradingsymbol, transaction_type, quantity, product, order_type, price=None,
        ...
        for k in list(params.keys()):
            if params[k] is None:
                del params[k]
        res = self.session.post(f"{self.root_url}/orders/{variety}",
    ...
    def modify_order(self, variety, order_id, parent_order_id=None, quantity=None, price=None, order_type=None,
        ...
        res = self.session.put(f"{self.root_url}/orders/{variety}/{order_id}",
                                    data=params, headers=self.headers).json()
        print(res)
        order_id= res["data"]["order_id"]
        return order_id
